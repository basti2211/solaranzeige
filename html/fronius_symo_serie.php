...
do {
  ...
  ****************************************************************************/
  ...
  $funktionen->log_schreiben( "API Version: ".$rc["APIVersion"]." CompatibilityRange: ".$rc["CompatibilityRange"], "   ", 3 );
  ...
  if ($rc["CompatibilityRange"] == "1.6-3" or $rc["CompatibilityRange"] == "1.5-18" or $rc["CompatibilityRange"] == "1.8-1") {
    ...
    if (isset($JSON_Daten["Head"]["Status"]["Code"]) and $JSON_Daten["Head"]["Status"]["Code"] == 0) {
      ...
      if ($WR_Adresse != $Teile[1]) {
        $funktionen->log_schreiben( "Wechselrichter Adresse ist nicht stimmig: ".$Teile[1], "   ", 3 );
      }
    ...
    $aktuelleDaten["Temperatur"] = 0;
    $funktionen->log_schreiben( "Modell = Fronius GEN 24", "   ", 3 );  
  }
  ...
  $JSON_Daten = $funktionen->read( $WR_IP, $WR_Port, $URL );
  // $funktionen->log_schreiben(print_r($JSON_Daten,1),"   ",1);
  if (isset($JSON_Daten["Head"]["Status"]["Code"]) and $JSON_Daten["Head"]["Status"]["Code"] == 0) {
    // Es handelt sich um gültige Daten
    $funktionen->log_schreiben( print_r( $JSON_Daten, 1 ), "   ", 10 );
  ...
  if ($aktuelleDaten["Meter"] > 0) {
    ...
    if (isset($JSON_Daten["Head"]["Status"]["Code"]) and $JSON_Daten["Head"]["Status"]["Code"] == 0) {
      ...
      if (isset($JSON_Daten["Body"]["Data"]["1"])) { //  Channel 1
  ...
  ****************************************************************************/
  if (file_exists( "/var/www/html/fronius_symo_serie_math.php" )) {
    include 'fronius_symo_serie_math.php'; // Falls etwas neu berechnet werden muss.
  }
...
Ausgang:$funktionen->log_schreiben( "-------------   Stop   fronius_symo_serie.php    --------------- ", "|--", 6 );
return;
?>
