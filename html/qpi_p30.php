...
do {
  ...
  for ($k = 1; $k < 200; $k++) {
    ...
    $Antwort .= $rc;
    if (empty($rc)) {
      usleep( 20000 ); // Muss auf 10000 bleiben. 2.5.2022 Die Effekta Geraete sind so langsam...
      continue;
    }
    $rc = "";
  ...
  if ($Wert === false) {
    $funktionen->log_schreiben( "Datenübertragung vom Wechselrichter war erfolglos! [Modus]", "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  if ($Wert === true) {
  ...
  if ($Wert === false) {
    $funktionen->log_schreiben( "Datenübertragung vom Wechselrichter war erfolglos! [Warnungen]", "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  if ($Wert === true) {
  ...
  usleep( 100000 ); //  [normal 10000] Es dauert etwas, bis die ersten Daten kommen ...
  for ($k = 1; $k < 200; $k++) {
    $rc = fgets( $USB, 4096 ); // 4096
    usleep( 10000 );
    $Antwort .= $rc;
  ...
  if ($Wert === false) {
    $funktionen->log_schreiben( "Datenübertragung vom Wechselrichter war erfolglos! [Standardwerte]", "    ", 5 );
    $rc = "";
    goto Ausgang;
  }
  ...
  if (substr( $Antwort, 1, 3 ) == "NAK") {
    ...
    $funktionen->log_schreiben( substr( $Antwort, 1, - 3 )."  i: ".$k, "    ", 1 );
