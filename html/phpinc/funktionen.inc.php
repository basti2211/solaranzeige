...
class funktionen {
  ...
  **************************************************************************/
  function influx_remote( $daten ) {
    if (!isset($daten["zentraleTimestamp"])) {
      $daten["zentralerTimestamp"] = time( );
    }
    $query = "";
  ...
  function query_erzeugen( $daten ) {
    ...
    switch ($daten["Regler"]) {
      ...
      case 23:
        ...
        $query .= "\n";
        // --------------------------
        if ($daten["SonoffModul"] == 201 or $daten["SonoffModul"] == 53) {
          $query .= "Summen ";
          $query .= "Wh_Heute=".$daten["WattstundenGesamtHeute"];
          $query .= ",Wh_Gesamt=".$daten["WattstundenGesamt"];
      ...
      case 44:
        ...
        /* Phocos Any-Grid  und EASUN SMG  ==>  1 Phasen Wechselrichter mit Batterie    */
      case 45:
      case 71:
        if (date( "i" ) == "01" or $daten["Demodaten"] or date( "H" ) == date( "H", $Sonnenaufgang )) {
          $query .= "Info ";
      ...
      case 48:
        ...
        if ($daten["Protokollversion"] == 3) {
          $query .= ",PV1_Leistung_Heute=".$daten["PV1_Leistung_Heute"];
          $query .= ",PV2_Leistung_Heute=".$daten["PV2_Leistung_Heute"];
          $query .= ",PV1_Leistung_Gesamt=".$daten["PV1_Leistung_Gesamt"];
          $query .= ",PV2_Leistung_Gesamt=".$daten["PV2_Leistung_Gesamt"];
        }
        $query .= "  ".$daten["zentralerTimestamp"];
        $query .= "\n";
      ...
      case 55:
        ...
        for ($i = 1; $i <= $daten["Anz-Xtender"]; $i++) {
          ...
          $query .= ",Entladestrom=".$daten["Inverter".$i."_BatterieEntladung"];
        ...
        /* Huawei WR M1 Modelle  , RCT WR , Sungrow WR  */
      case 56:
      case 62:
      case 65:
      case 70:
      ...
      case 78:
        ...
        $query .= ",MaxAmpere=".($daten["MaxLadestrom"]);
        $query .= ",Ladekabel=".$daten["Kabelstatus"];
  ...
  function eSmart3_auslesen( $Device, $Befehl = '' ) {
    ...
    do {
      ...
      if (trim( $Antwort ) == "") {
        ...
        $Ergebnis .= $Antwort;
        $this->log_schreiben( bin2hex( $Ergebnis )." Länge: ".strlen( $Ergebnis ), "   ", 10 );
  ...
  **************************************************************************/
  function phocos_pv18_auslesen( $USB, $Input, $Timer = "50000" ) {
    ...
    for ($k = 1; $k < 15; $k++) { // vorher 3
      ...
      usleep( $Timer ); // 0,05 Sekunden warten
      for ($i = 1; $i < 10; $i++) {
        $buffer .= fgets( $USB, 1024 );
        usleep( 50000 );
        $buffer .= fgets( $USB, 1024 );
        ...
        if (substr( bin2hex( $buffer ), 0, 4 ) == "0103" or substr( bin2hex( $buffer ), 0, 4 ) == "0104" or substr( bin2hex( $buffer ), 0, 4 ) == "0403") {
        ...
        if (substr( bin2hex( $buffer ), 0, 4 ) == $Input["DeviceID"]."83") {
          $data["lenght"] = 1;
          $data["data"] = 0;
          $data["ok"] = true;
          // echo "Fehler! ..\n";
        ...
        elseif (bin2hex( $this->crc16( substr( $buffer, 0, - 2 ))) == bin2hex( substr( $buffer, - 2 ))) {
  ...
  ****************************************************************************/
  function modbus_tcp_lesen( $COM1, $GeraeteAdresse, $FunktionsCode, $RegisterAdresse, $RegisterAnzahl, $DatenTyp, $Timebase = 600000 ) {
    ...
    $MessageLenght = str_pad( dechex( strlen( $GeraeteAdresse.$FunktionsCode.$RegisterAdresse.$RegisterAnzahl ) / 2 ), 4, "0", STR_PAD_LEFT );
    for ($j = 0; $j < 5; $j++) {
      ...
      elseif ($Daten["Befehl"] == 83) {
        $this->log_schreiben( "3 Sekunden Pause", "   ", 5 );
        sleep( 3 );
        continue;
      }
